...
async function renderCards() {
     ...
     const data = await fetch(`https://gateway.marvel.com:443/v1/public/characters?apikey=9ab871748d83ae2eb5527ffd69e034de`)
          .then(res => res.json()) //Converting the data into JSON format
          .then(data => data.data.results) //sending the searched results characters to show in HTML
     if (data.length > 0) {
          ...
          data.forEach(hero => {
               cardsHTML += `                         
               <li class="flex-row single-search-result m-3 d-flex flex-column" style="min-width:340px;">
               <div class="flex-row img-info d-flex flex-column align-items-center justify-content-center ">
                    <img src="${hero.thumbnail.path + '/portrait_medium.' + hero.thumbnail.extension}" alt="${hero.name}">
                    <div class="hero-info">
               ...
               <div class="flex-col buttons">
                    <button class="btn add-to-fav-btn">${favouritesCharacterIDs.includes(`${hero.id}`) ? "<i class=\"fa-solid fa-heart-circle-minus\"></i> &nbsp; Remove from Favourites" : "<i class=\"fa-solid fa-heart fav-icon\"></i> &nbsp; Add to Favourites</button>"}
               </div>
...
function showSearchedResults(searchedHero) {
     ...
     if(searchedHero.length >0){
          searchResults.style.marginBottom = "-450px"
     }else{
          searchResults.style.marginBottom = "0px"  
     }
